...
export default function App() {
  ...
  const playerRefs = useRef<{ [id: string]: any }>({});

  // Update speed of all players when globalSpeed changes
  useEffect(() => {
  ...
  // Update mute state of all players when globalMute changes
  useEffect(() => {
    Object.values(playerRefs.current).forEach(player => {
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full relative z-10">
        ...
        <header className="h-16 border-b border-zinc-800/50 bg-[#0a0a0a]/80 backdrop-blur-md flex items-center justify-between px-4 shrink-0">
          ...
          <div className="flex items-center gap-3 ml-4">
            {/* Global Controls */}
            <div className="flex items-center gap-2 bg-zinc-900/50 border border-zinc-800 rounded-md p-1">
              <button
              ...
              <div className="relative group flex items-center">
                <button
                  className="p-1.5 rounded transition-colors flex items-center gap-2 px-3 text-zinc-300 hover:text-cyan-400 hover:bg-cyan-400/10"
                ...
                <div className="absolute top-full left-0 mt-1 bg-zinc-900 border border-zinc-800 rounded-md shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 py-1 min-w-[100px]">
                  {[0.5, 1, 1.25, 1.5, 1.75, 2].map(speed => (
                    <button
                      key={speed}
                      onClick={() => setGlobalSpeed(speed)}
                      className={`w-full text-left px-4 py-2 text-sm hover:bg-zinc-800 transition-colors ${globalSpeed === speed ? 'text-cyan-400 font-bold' : 'text-zinc-300'}`}
                    >
                      {speed}x Speed
                    </button>
                  ))}
                    ...
                    {videos.map((video) => (
                      ...
                      >
                        ...
                        <div className="flex flex-1 w-full h-full">
                          <div className={`${video.showChat ? 'w-[70%]' : 'w-full'} h-full`}>
                            <YouTube
                              ...
                              opts={{
                                ...
                                playerVars: {
                                  autoplay: 1,
                                  mute: (globalMute || (focusedVideo && focusedVideo !== video.id)) ? 1 : 0,
                                  playsinline: 1
                                },
                              }}
                              ...
                              onReady={(event) => {
                                ...
                                try {
                                  event.target.setPlaybackRate(globalSpeed);
                                } catch (e) {}
                              }}
                            />
                          </div>
                ...
                <div className="absolute inset-0 z-50 bg-[#050505]/95 backdrop-blur-xl p-4 sm:p-6 flex flex-col">
                  ...
                  <div className="flex-1 w-full max-w-7xl mx-auto rounded-xl overflow-hidden border border-zinc-800 shadow-2xl bg-black">
                    <div className="w-full h-full">
                      <YouTube
                        videoId={videos.find(v => v.id === focusedVideo)?.videoId || ''}
                        opts={{
                          width: '100%',
                          height: '100%',
                          playerVars: {
